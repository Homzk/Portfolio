...
- [~] T043 Grabar miniaturas `webm` reales + `poster` en `public/media/` con la receta `ffmpeg` del quickstart. **Hecho: Maderas + AirVision** (grabados de los sitios en vivo con `scripts/record-thumbs.mjs` → VP9 720×450 + poster, activados en `VIDEO_READY`). **Pendiente: LPR** (footage en terreno) **y Eventos** (app móvil capstone) — siguen con placeholder animado hasta tener footage de Álvaro
